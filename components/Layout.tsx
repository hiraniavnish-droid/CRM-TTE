...
export const Layout = () => {
  ...
  const getBackground = () => {
    switch (theme) {
      ...
      case 'light':
        return 'bg-transparent'; // GridBackground handles the base
      default:
  ...
  const navItems = [
    ...
    { path: '/builder', label: 'Itinerary Builder', icon: Map }, // New Item
  ...
  return (
    <GridBackground className={cn('transition-colors duration-300 ease-in-out font-sans', getBackground())}>
      ...
      <aside className={cn(
        ...
        {/* Navigation - Clean Floating Pill Style */}
                ...
                {isActive && (
                  <MotionDiv
                    ...
                    className={cn(
                      ...
                      theme === 'light' 
                        ? "bg-blue-50 border border-blue-100 shadow-md shadow-blue-100/50" 
                        : "bg-blue-500/10 border border-blue-500/20 shadow-none"
                    )}
                ...
                >
                    <item.icon 
                        ...
                        className={cn(
                            ...
                            isActive ? "text-current" : "opacity-70 group-hover:opacity-100 group-hover:text-current"
                        )} 
        ...
        <div className={cn("p-6 border-t", theme === 'light' ? 'border-slate-100' : 'border-slate-800', "space-y-2")}>
          <button 
            ...
            className={cn(
                ...
                theme === 'light' ? "text-slate-500 hover:text-red-600 hover:bg-red-50" : "text-slate-400 hover:text-red-400 hover:bg-white/5"
            )}
          >
              <LogOut size={18} />
              Logout
          ...
          <div className="flex gap-1 p-1 rounded-lg bg-slate-100 border border-slate-200 mt-2">
            {[
              { id: 'light', icon: Sun },
              { id: 'dark', icon: Moon },
              { id: 'ocean', icon: Droplets }
            ].map((t) => (
              <button 
                key={t.id}
                onClick={() => setTheme(t.id as any)}
                className={cn(
                  "flex-1 p-1.5 rounded-md flex justify-center transition-all duration-200", 
                  theme === t.id 
